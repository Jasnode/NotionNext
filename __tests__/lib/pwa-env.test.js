...
jest.mock('@/blog.config', () => ({
  ...
  PWA_ENABLE: false,
  PWA_NAME: 'Env install name',
  PWA_SHORT_NAME: 'Env install',
  PWA_DESCRIPTION: 'Env install description',
  PWA_THEME_COLOR: '#123456',
  PWA_BACKGROUND_COLOR: '#abcdef',
}))
...
describe('PWA env var fallback', () => {
  it('uses BLOG.PWA_ICON_192 as fallback when notionConfig is empty', () => {
    const config = getPwaConfig({
      siteInfo: { title: 'Site title', icon: '/site-icon.png' },
      notionConfig: {}
    })
    expect(config.name).toBe('Env install name')
    expect(config.shortName).toBe('Env install')
    expect(config.description).toBe('Env install description')
    expect(config.icon).toBe('/env-icon.png')
    expect(config.themeColor).toBe('#123456')
    expect(config.backgroundColor).toBe('#abcdef')
  ...
  it('notionConfig takes priority over BLOG (env var) values', () => {
    const config = getPwaConfig({
      siteInfo: {},
      notionConfig: {
        PWA_NAME: 'Notion install name',
        PWA_SHORT_NAME: 'Notion install',
        PWA_DESCRIPTION: 'Notion install description',
        PWA_ICON: '/notion-icon.png',
        PWA_ICON_192: '/notion-192.png',
        PWA_THEME_COLOR: '#654321',
        PWA_BACKGROUND_COLOR: '#fedcba'
      }
    })
    expect(config.name).toBe('Notion install name')
    expect(config.shortName).toBe('Notion install')
    expect(config.description).toBe('Notion install description')
    expect(config.icon).toBe('/notion-icon.png')
    expect(config.themeColor).toBe('#654321')
    expect(config.backgroundColor).toBe('#fedcba')
