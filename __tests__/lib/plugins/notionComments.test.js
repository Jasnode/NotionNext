...
describe('notionComments helpers', () => {
  ...
  test('filters non-public comments', () => {
    ...
    expect(isPublicComment({ status: '' }, true)).toBe(false)
  })

  test('requires a writable Status property when approval is enabled', () => {
    expect(
      buildCommentStatusProperty(
        { Status: { type: 'select' } },
        'Pending',
    ...
    ).toEqual({ select: { name: 'Pending' } })
    expect(() =>
      buildCommentStatusProperty({}, 'Pending', true)
    ).toThrow('Status')
  })
