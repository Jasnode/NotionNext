...
export function getPageContentTextList(post, pageBlockMap) {
  /**
   * 将对象的指定字段拼接到字符串
   * @param block
  ...
  function getPostContentIds() {
    if (Array.isArray(post.content) && post.content.length > 0) {
      return post.content
    }

    const rootContent = pageBlockMap?.block?.[postId]?.value?.content
    if (Array.isArray(rootContent) && rootContent.length > 0) {
      return rootContent
    }

    if (!pageBlockMap?.block) {
      return []
    }

    return Object.keys(pageBlockMap.block).filter(
      id => pageBlockMap.block[id]?.value?.parent_id === postId
    )
  }

  const postId = post.id
  const postContent = getPostContentIds()
  let contentTextList = []
  // 防止搜到加密文章的内容
...
export function getPageContentText(post, pageBlockMap) {
  return getPageContentTextList(post, pageBlockMap).join('')
}
