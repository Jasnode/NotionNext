...
export const getPwaConfig = ({ siteInfo, notionConfig } = {}) => {
  ...
  const description =
    ...
    BLOG.PWA_DESCRIPTION ||
    siteInfo.description ||
    notionConfig.DESCRIPTION ||
    title
  const icon = notionConfig.PWA_ICON || BLOG.PWA_ICON || siteInfo.icon || DEFAULT_ICON
  ...
  const backgroundColor =
    notionConfig.PWA_BACKGROUND_COLOR || BLOG.PWA_BACKGROUND_COLOR || '#ffffff'
  const themeColor =
    notionConfig.PWA_THEME_COLOR || BLOG.PWA_THEME_COLOR || backgroundColor

  return {
    name: title,
    shortName: notionConfig.PWA_SHORT_NAME || BLOG.PWA_SHORT_NAME || title,
    description,
    icon,
    icon192,
    icon512,
