...
const countUnits = [
  { threshold: 100000000, suffix: 'E' },
  { threshold: 10000, suffix: 'W' },
...
const formatCount = value => {
  ...
  const unit = countUnits.find(item => count >= item.threshold)
  const rounded = Math.round((count * 100) / unit.threshold) / 100
    ...
    this.bszs.map(function (key) {
      ...
      if (elements) {
        for (var element of elements) {
          element.textContent = formatCount(data[key]);
        }
      }
    })
  },
