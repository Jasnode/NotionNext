...
const isPublicComment = (comment, requireApproval = false) =>
  requireApproval
    ? comment.status === PUBLIC_COMMENT_STATUS
    : !comment.status || comment.status === PUBLIC_COMMENT_STATUS

const buildCommentStatusProperty = (
  properties,
  status,
  ...
  if (requireApproval) {
    throw new Error(
      'NOTION_COMMENT_REQUIRE_APPROVAL requires a Status select or status property'
    )
  }
  return null
}
