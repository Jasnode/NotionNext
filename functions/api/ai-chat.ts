...
const isMessage = (message: unknown): message is UIMessage =>
...
export const onRequestPost = async ({ request, env }: PagesContext) => {
  ...
  if (
    typeof env.AI_CHAT_API_KEY !== 'string' ||
    !env.AI_CHAT_API_KEY.trim()
  ) {
    return json(
  ...
  if (
    ...
    (!Array.isArray(body.messages) || body.messages.some(message => !isMessage(message)))
  ) {
    return json({ error: 'Invalid messages.' }, { status: 400, headers })
  }
  ...
  try {
    const response = await fetch(
      completionUrl(env.AI_CHAT_BASE_URL || DEFAULT_BASE_URL),
      {
        method: 'POST',
        headers: {
          authorization: `Bearer ${env.AI_CHAT_API_KEY.trim()}`,
    ...
    return json({ text: typeof text === 'string' ? text : '' }, { headers })
  } catch (error) {
    console.error(error)
